      ...
      <div class="max-w-md w-full space-y-8">
        <div class="bg-white dark:bg-gray-800 py-8 px-6 shadow-lg rounded-lg border dark:border-gray-700">
          ...
          <form @submit.prevent="handleRegister" class="space-y-6">
            ...
            <div v-if="info" class="text-blue-600 dark:text-blue-400 text-sm">
              {{ info }}
            </div>
          ...
          <div class="mt-6 text-center">
            <p class="text-gray-600 dark:text-gray-400">
              ...
              <NuxtLink to="/login" class="text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 font-medium">
                Login here
              </NuxtLink>
            </p>
          </div>
...
const error = ref('')
const info = ref('The first registered user will automatically become a Manager.')
...
const handleRegister = async () => {
  ...
  // Enhanced validation using composable
  ...
  try {
    ...
    const result = await authStore.register(validation.normalizedEmail, form.value.password)
    
    if (result.success) {
      toast.success('Registration Successful!', 'Welcome to FinanceFlow')
      await safePush('/dashboard')
    } else {
